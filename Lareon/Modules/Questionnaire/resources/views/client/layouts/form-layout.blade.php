<form id="form-{{$form->id}}" class="{{isset($ajax) && $ajax ? 'formMode' : '' }}" action="{{route('client.submitting.form')}}" method="POST" id="{{uuid_create().rand(10,100)}}" {{$form->has_file ? 'enctype="multipart/form-data"' : ''}}>
    @csrf
    ...
    <input type="text" class="hidden" name="formpot">
    @if($form->template)
        <div>
