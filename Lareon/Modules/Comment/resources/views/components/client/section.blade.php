...
<section id="comment-sec">
    ...
    @if(config('lareon.comment.unconfirmed_visibility'))
        <x-comment::client.unconfirmed :comments="$unconfirmed" :model="$model"/>
    @endif
    <div>
        <x-comment::client.new :model="$model"/>
        <hr class="border-zinc-300 mx-auto my-6 w-11/12">
        <ul class="space-y-6" id="listComment">
            <x-comment::client.confirmed :comments="$comments ?? []" :model="$model"/>
        </ul>
        <x-comment::client.reply :model="$model" id="comment_modal"/>
        <<x-comment::client.more :model="$model" :offset="$offset" :limit="$limit" id="comment_modal"/>
    </div>
</section>
