...
class SchemaGeneratorService
{
    ...
    {
        $method = isset($this->schema['seo_type']) && self::SCHEMA_TYPES[$this->schema['seo_type']] ?  self::SCHEMA_TYPES[$this->schema['seo_type']] : null ;
        if (!$method || !method_exists($this, $method)) return [$this->generateWebPage()];
    ...
    private function generateWebPage(): WebPage
    {
        ...
        return Schema::webPage()
            ...
            ->breadcrumb($this->createBreadcrumb($this->instance['breadcrumb'] ?? $this->model?->breadcrumb() ?? []));
    }
    ...
    private function createBreadcrumb(array $data): ?BreadcrumbList
    {
        ...
        return count($items)
            ? Schema::breadcrumbList()->itemListElement($items)
            : null;
    }
}
