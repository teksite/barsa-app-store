...
trait SeoAble
{
    ...
    public function saveSeo(array $input =[])
    {
        $this->addToSitemap($input['sitemap'] ?? []);
        $this->addToMetaTag($input['meta'] ?? [] , $input['schema'] ?? []);

    }

    public function deactiveSeo()
    {
        $this->deactivateInSitemap();
        //$this->addToMetaTag($input['meta'] ?? [] , $input['schema'] ?? []);
