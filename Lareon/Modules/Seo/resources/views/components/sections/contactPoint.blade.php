...
<x-lareon::accordion.box :open="$open" :accordion="$accordion">
    <div>
            ...
            @foreach($value ?? [] as $contact )
                <div id="contactItem-{{$loop->index}}" class="p-1 contactItem mb-6 border border-zinc-300">
        ...
        fields: [],
            ...
            <div>
                <template x-data="{'lngth' : document.querySelectorAll('.contactItem').length}" x-for="(field, index) in fields" :key="index">
                    <div class="p-1 contactItem mb-6 border border-zinc-300" x-bind:id="`contact-${index + lngth + 1}`">
