...
<x-lareon::accordion.box :title="__('publish status')" :open="$open" :accordion="false">
    ...
    <x-lareon::input.select id="publishStatus_{{$random}}" class="block mt-1 w-full" name="publish_status"
                            aria-label="{{__('publish status')}}">
        @foreach(\Lareon\CMS\App\Enums\PublishStatusEnum::cases() as $case)
            <option
                value="{{ $case->value }}"
                {{ (old('publish_status', $value[0]?->value ?? null) == $case->value) ? 'selected' : '' }}
            >
                {{ __($case->getName()) }}
            </option>
        @endforeach
    ...
    <x-lareon::input.time id="publishAtInput_{{$random}}" type="datetime-local" name="published_at"
                          :value="old('published_at', $value[1] ?? '')" class="block w-full"/>
