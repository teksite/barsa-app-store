...
class UserLogic
{
    ...
    public function change(array $input, User $user): ServiceResult
    {
        return app(ServiceWrapper::class )(function () use ($input, $user) {
            ...
            if(request()->routeIs('admin.*') && auth()->id() != $user->id){
                $user->assignRole($input['roles'] ?? $user->roles()->pluck('id')->toArray());
                $user->syncPermissions($input['permissions'] ?? []);
            }
            $user->saveSeo($input['seo'] ?? []);
    ...
    {
        return app(ServiceWrapper::class)(function () use ($input, $user){
            $user->update(Arr::only($input, ['password']));
            return $user;
        });
    }
