@props([ 'name', 'show' => false,  'maxWidth' => '2xl' ,'id'=>rand(100,10000) .'id'])
@php
    $maxWidth = [
        'sm' => 'sm:max-w-sm',
        'md' => 'sm:max-w-md',
...
<div id="{{$id}}"
    ...
    x-show="show"
    class="fixed inset-0 overflow-y-auto p-6 sm:px-0 z-50 flex items-center justify-center" style="display: {{ $show ? 'block' : 'none' }};">
    <div x-show="show" x-on:click="show = false" x-transition:enter="ease-out duration-300"
         x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
         x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100"
         x-transition:leave-end="opacity-0"
         class="fixed inset-0 transform transition-all">
        <div class="absolute inset-0 bg-zinc-900 opacity-90 backdrop-blur-2xl"></div>
